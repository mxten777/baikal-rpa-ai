...
export default function Layout() {
    ...
    <>
      ...
      <div className="px-5 py-5 flex items-center justify-between border-b border-white/10">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-gradient-to-br from-baikal-400 to-baikal-600 rounded-lg flex items-center justify-center text-white font-black text-sm shadow-lg">
            B
          </div>
          <div>
            <div className="text-base font-bold tracking-wide leading-tight">BAIKAL RPA AI</div>
            <div className="text-[10px] text-baikal-300 tracking-wider">업무 자동화 플랫폼</div>
          </div>
        </div>
      ...
      <nav className="flex-1 px-3 py-5 space-y-1 overflow-auto">
        <div className="text-[10px] uppercase tracking-widest text-baikal-400 px-3 mb-3 font-semibold">Main Menu</div>
        {MAIN_NAV.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            ...
            className={({ isActive }) =>
              `group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-200 ${
                isActive 
                  ...
                  : 'text-baikal-200 hover:bg-white/8 hover:text-white'
              }`
            }
          >
            <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center group-hover:bg-white/15 transition">
              <item.icon size={16} />
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-medium leading-tight">{item.label}</div>
              <div className="text-[10px] text-baikal-400 group-hover:text-baikal-300 truncate">{item.desc}</div>
            </div>
          </NavLink>
        ))}

        <div className="text-[10px] uppercase tracking-widest text-baikal-400 px-3 mt-6 mb-3 font-semibold">Quick Actions</div>
        <button 
          onClick={() => navigate('/documents/new')}
          className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm text-baikal-200 hover:bg-white/8 hover:text-white transition"
        >
      ...
      <div className="border-t border-white/10 px-3 py-3">
        <div className="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-white/8 transition cursor-pointer" onClick={() => setShowUserMenu(!showUserMenu)}>
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-baikal-400 to-baikal-600 flex items-center justify-center text-xs font-bold shrink-0">
            {user?.name?.[0] || 'U'}
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium truncate">{user?.name || '사용자'}</div>
            <div className="text-[10px] text-baikal-400 truncate">{user?.email || ''}</div>
          </div>
          <FiChevronDown size={14} className="text-baikal-400" />
        </div>
        {showUserMenu && (
          <div className="mt-1 space-y-1">
            <button onClick={logout} className="w-full flex items-center gap-2 px-4 py-2 rounded-lg text-sm text-red-300 hover:bg-red-500/20 transition">
              <FiLogOut size={14} />
              로그아웃
            </button>
          </div>
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-baikal-900 to-baikal-950 text-white flex flex-col shadow-xl
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        {/* Top Bar */}
        <header className="h-14 bg-white border-b flex items-center justify-between px-4 sm:px-6 shrink-0 shadow-sm">
